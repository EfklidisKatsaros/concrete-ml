...
def mean_squared_error(y_true: numpy.ndarray, y_pred: numpy.ndarray):
    ...
    Arguments:
        ...
        y_pred (numpy.ndarray): The computed logits.
...
class LinearRegressionTraining(torch.nn.Module):
    """Linear Regression training module.
    ...
    def forward(
        ...
        for _ in range(self.iterations):
            ...
            # Compute the gradients
            derive_z = logits - targets  # (n_samples, n_targets, 1)
            derive_weights = (
                features.transpose(1, 2) @ derive_z / features.size(1)
            )  # (1, n_features, 1)
